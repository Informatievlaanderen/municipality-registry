namespace MunicipalityRegistry.Api.Extract.Extracts
{
    ...
    public class ExtractController : ApiController
    {
        ...
        [SwaggerResponseExample(StatusCodes.Status200OK, typeof(MunicipalityRegistryResponseExample))]
        [SwaggerResponseExample(StatusCodes.Status500InternalServerError, typeof(InternalServerErrorResponseExamples))]
        public IActionResult Get(
            [FromServices] ExtractContext context,
            CancellationToken cancellationToken = default) =>
            new IsolationExtractArchive($"{ZipName}-{DateTime.Now:yyyy-MM-dd}", context) { MunicipalityRegistryExtractBuilder.CreateMunicipalityFile(context) }
